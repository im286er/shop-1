		...
		try {
			...
			if ($this->isPost ()) {
				...
				if ($SP->parseSearchInfo ( true )) {
					...
					// var_dump($Url);
					$RedirectUrl = __APP__ . '/product-index-' . substr ( $Url ['url'], 1 ).'-s-1';
					// echo $RedirectUrl;
					// exit;
...
//					exit;
					if ($result){
					    ...
					    $Url = $SP->getFormattedUrl_new ();
					    $BaseUrl = __APP__ . '/product-index-' . substr ( $Url ['url'], 1 );
			...
			$this->assign('slshownav',1);//sl:sell well lists
			//为Nav显示高亮新增参数-End
