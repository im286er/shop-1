 ...
 * Time: 14:44
 */
class IwxAction extends CommonAction{
    ...
    public function jewelrylist()
    {
       ...
       /* echo "<pre/>";
        print_r($PSM->getResult($SP->SearchInfo ['page'],2));
        exit;*/
        $this->assign('SearchResult', $PSM->getResult($SP->SearchInfo ['page'],2));
        ...
        $this->assign('wxtitle', "IGNITE 创意首饰");
        $this->_renderPage();
    }
    ...
    public function jewelrylistajax()
    {
        ...
        /*******************判断是否为轻定制************************/
        if($type == 'diy'){
            $result = $PSM->getResult($SP->SearchInfo ['page'],3);
        }else{
...
//    }
    ...
    {
        $pid = I('pid', 0, 'intval');
        $userAgent = M('user_prepaid')->where('p_id=' . $pid)->getField('up_agent_userid');
        if ($userAgent == '0') {
            session_start();
            ...
            //获取产品图片信息
            $messArr['cover'] = M('product')->where('p_id=' . $pid)->getField('p_cover');
            //获取分享人的用户ID
            $messArr['s_uid'] = $uid;
        }
        $messArr['pid'] = $pid;
    ...
    public function showletter(){
        ...
        $letter=urldecode(I('code','0','string'));
