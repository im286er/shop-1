...
class ModelsAction extends CommonAction {
	...
	public function detail() {
		...
		try {
			...
			if ($Product) {
				...
				$showtitle=$this->catechild($Category)."-".$Product['p_name'];
	...
	public function index() {
		try {
			...
			// 分类筛选,去除分页参数
			$cate_url = trim_slash_url ( preg_replace ( '|-cate-(\d*)|', '', __SELF__ ) );
			...
			// 搜索工具筛选
			preg_match ( '|filter-([a-z]+)\-|', __SELF__, $matches );
			$SearchInfo_url = process_filter_slash_url ( process_filter_page_url ( __SELF__ ) );
			$filter_url = array ();
			if (count ( $matches ) > 0) {
			...
			// 清除所有筛选条件
			$clear_all_url = preg_replace ( '|-filter-(.*)-|', '', __SELF__ );
			...
			$paging_url ['35'] = trim_slash_url ( preg_replace ( '|-count-(\d*)|', '', __SELF__ ) ) . '-count-35';
			$paging_url ['45'] = trim_slash_url ( preg_replace ( '|-count-(\d*)|', '', __SELF__ ) ) . '-count-45';
			$paging_url ['60'] = trim_slash_url ( preg_replace ( '|-count-(\d*)|', '', __SELF__ ) ) . '-count-60';
			$paging_url ['75'] = trim_slash_url ( preg_replace ( '|-count-(\d*)|', '', __SELF__ ) ) . '-count-75';
			...
			// 每页显示(新版)
			if ($SearchInfo ['thumb'] == 1) { // 大图模式
				$count_opt_arr = C ( 'SEARCH.RES_COUNT_OPTION3' );
			} else {
	...
	private function verificationRequest($RequestVM, $AutoJump = true) {
		...
		if ($PID) {
			...
			return $AutoJump ? $this->error ( L ( 'POST_VALUE_ERROR' ), U ( 'Front/index' ) ) : false;
		}
	}
