...
class LoginAction extends CommonAction {
	...
	public function wx() {
		...
		$jump_url = $refererUrl;
		if ($jump_url) {
			$redirect_uri = WX_CALLBACK_DOMAIN . "/index/auth-wx_oauth?entry=open&from_uri=" . $jump_url;
		} else {
			$redirect_uri = WX_CALLBACK_DOMAIN . "/index/auth-wx_oauth?entry=open";
		}
		
		//
		$WX = new Wxsdk ( 'open' );
		$wxLoginUrl = $WX->genWxLoginUri ( $redirect_uri );
	...
	public function sso() {
		...
		// 配置信息
		$CFG_REMOTE_HOST = "http://118.126.8.224/";
		$CFG_LOCAL_HOST = "http://www.ignjewelry.com/";
		// $CFG_LOCAL_CHECKLOGIN = "http://127.0.0.1/checklogin.html";
		$CFG_LOCAL_CHECKLOGIN = array (
				0 => "http://www.ignjewelry.com/checklogin.html",
				1 => "http://www.ignjewelry.com/checklogin.html",
				2 => "http://www.ignjewelry.com/checklogin2.html"
		);
	...
	public function index() {
		...
		try {
			...
			if ($this->isPost ()) {
				...
				// 登录成功后的跳转路径
				$from_url = empty($from_url) ? WEBROOT_URL : $this->_post ( 'from_url' );
				if($from_url == WEBROOT_URL){
					if($type == 'iwx'){
						$from_url = WEBROOT_URL.'/index.php/iwx';
					}
				}

				//上传至外网需要打开,验证码判断
//				if($this->_post('login_type')=='2'){
//					if (strtolower ( $this->_post ( 'verification' ) ) !== strtolower ( $this->_session ( 'php_captcha' ) )) {
//						$data['code']=2;
//						$data['from_url']=$from_url;// 用户名密码验证成功
...
//				}
				...
				// 用户名密码判断
				$condition ['u_email'] = $this->_post ( 'phone' );
				$Users = new UsersModel ();
				$res = $Users->where ( $condition )->find ();
				if($res == NULL){
					$condition_ ['u_mob_no'] = $this->_post ( 'phone' );
					$res = $Users->where($condition_)->find();
				}
				if ($res){
					//用户名验证成功
					$uid = $Users->u_id;
					$md5pass = md5 ( $this->_post ( 'password' ). $Users->u_salt );
					if ($Users->u_pass === $md5pass) { // 新增密码验证规则，需统一为md5(md5(passwo  rd).盐值)
						...
						//$newOrderId=$Users->haveNewOrder(session ( 'f_userid' ));
						/*if($newOrderId){
							//phpalert('您有新的订单！','jump',WEBROOT_URL."/user.php/sales/orderdetail/orderid/".$newOrderId);
							$data['code']=1;
							$data['from_url']=WEBROOT_URL."/user.php/sales/orderdetail/orderid/".$newOrderId;
							$this->ajaxReturn($data,JSON);
				...
				//$from_url = urldecode ( get_url());
				$from_url = (!$this->_get('from_url')) ? WEBROOT_URL :urldecode($this->_get('from_url'));
